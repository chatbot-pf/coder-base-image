...
- 접속 방식 선택:
  ...
  3. **coder port-forward / SSH 포워딩** — 개인 데스크톱 용도의 최소 구성
...
## 대안: coder port-forward / SSH 포워딩

서브도메인 앱이나 포트 publish 없이 개인 용도로만 쓸 경우.
이때 startup script는 `--pairing-address 127.0.0.1 --port 6768`로 실행합니다.
...
`coder config-ssh`를 설정해 두었다면 표준 SSH 포트 포워딩도 동일하게 동작합니다
(둘 다 Coder의 터널 위로 흐르므로 기능적으로 같습니다):

```bash
ssh -L 6768:localhost:6768 coder.<workspace>
```

매번 명령을 유지하기 번거로우면 `~/.ssh/config`에 등록해 SSH 접속 시 자동 포워딩:

```
Host coder.<workspace>
  LocalForward 6768 localhost:6768
```
